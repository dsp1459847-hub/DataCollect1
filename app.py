...
class SattaPredictiveEngine:
    ...
    def run_prediction(self, target_date, shift, top_n_jodis, window_size=120):
        target_dt = pd.to_datetime(target_date)

        if shift not in self.df.columns:
            return None

        hist_df = self.df[self.df["DATE"] <= target_dt].copy()
        if hist_df.empty:
            return None
        ...
        if valid_count < 5:
            return {
                "error": f"⚠️ {shift} channel में valid records बहुत कम हैं: {valid_count}.",
                "valid_count": valid_count
            }

        last_val = int(series[-1])
        weights = np.linspace(0.7, 1.3, num=len(series)) if len(series) > 1 else np.array([1.0])
        ...
        for num in range(100):
            base = 0.52 * markov_scores[num] + 0.28 * rashi_scores[num] + 0.20 * gap_scores[num]
            recency_boost = 1.0 + (((target_dt.day + target_dt.month + num) % 5) * 0.01)
            final_scores[num] = base * recency_boost
            t, u = num // 10, num % 10
        ...
        return {
            ...
            "prediction_date": (target_dt + pd.DateOffset(days=1)).strftime("%Y-%m-%d"),
...
if not available_shifts:
    st.error("❌ कोई valid channel column नहीं मिला.")
    st.stop()

if "selected_shift" not in st.session_state:
    st.session_state.selected_shift = available_shifts[0]

if "selected_date" not in st.session_state:
    st.session_state.selected_date = raw_df["DATE"].dt.strftime("%Y-%m-%d").iloc[-1]

if "jodi_count" not in st.session_state:
    st.session_state.jodi_count = 6

if "window_size" not in st.session_state:
    st.session_state.window_size = 120

st.sidebar.header("⚙️ एल्गोरिदम सेटिंग्स")
selected_shift = st.sidebar.selectbox(
    "🎯 शिफ्ट (Shift/Channel) चुनें:",
    available_shifts,
    index=available_shifts.index(st.session_state.selected_shift) if st.session_state.selected_shift in available_shifts else 0,
    key="selected_shift"
)
...
selected_date = st.sidebar.selectbox(
    ...
    all_dates,
    index=all_dates.index(st.session_state.selected_date) if st.session_state.selected_date in all_dates else len(all_dates) - 1,
    key="selected_date"
)

jodi_count = st.sidebar.slider("🔢 जोड़ियों की संख्या (Top Jodis):", 5, 10, st.session_state.jodi_count, key="jodi_count")
window_size = st.sidebar.slider("🪟 Rolling Window Size:", 30, 365, st.session_state.window_size, key="window_size")
...
if st.button("🚀 अगले दिन की भविष्यवाणी (Prediction) शुरू करें", key="predict_btn"):
    ...
    if results:
        ...
        with col2:
            ...
            st.subheader("🎯 महा-धमाका सिंगल हरूफ")
    ...
    st.info("Prediction शुरू करने के लिए बटन दबाइए. हर change पर widget state अलग रहेगा.")
